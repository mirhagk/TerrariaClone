...
namespace TerrariaClone
{
    public class World
    {
        ...
        static bool[,] coordlist;
        static bool[,] coordlist2;
        ...
        public static Object[] generateChunk(int cx, int cy, Random random)
        {
            ...
            for (y = 0; y < size; y++)
            {
                for (x = 0; x < size; x++)
                {
                    ...
                    blockdns[y,x] = (byte)random.Next(5);
                    blockbgs[y,x] = 0;
                    blockts[y,x] = (byte)random.Next(8);
                    lights[y,x] = (float)19;
                    lsources[y,x] = false;
                    wcnct[y,x] = false;
                    drawn[y,x] = false;
        ...
        public static Byte[,] generateOutlines(int[,] blocks)
        {
        ...
        /*    public static Object[] generate(int width, int height, int sealevel, int stonelevel, TerrariaClone inst) {
                ...
                int[][] blocksB = new int[height][width];
                for (y=0; y<height; y++) {
                    for (x=0; x<width; x++) {
                        blocksB[y][x] = 0;
                    }
                }
                int[][] blocksF = new int[height][width];
                for (y=0; y<height; y++) {
                    for (x=0; x<width; x++) {
                        blocksF[y][x] = 0;
                    }
                ...
                bool[] verify = {false, false, false, false, false};
                bool bvar = false;
                while (true) {
                    bCount = 0;
                    for (i=0; i<5; i++) {
                ...
                int[][][] blocks_rv = {blocksB, blocks, blocksF};

                Object[] rv = {blocks_rv, new DoubleContainer(terrain), new DoubleContainer(stonelayer)};
                return rv;
            }
        */
        public static Byte[,] generate2(int[,] blocks, bool msg)
        {
            int x, y;
            int width = blocks.GetUpperBound(1);
            ...
            bool left, right, up, down, upleft, upright, downleft, downright;
            for (y = 0; y < height; y++)
            {
        ...
        public static Byte[,] generate2b(int[,] blocks, Byte[,] blockds, int xpos, int ypos)
        {
            int x, y;
            int width = blocks.GetUpperBound(1);
            int height = blocks.GetUpperBound(0);
            bool left, right, up, down, upleft, upright, downleft, downright;
            left = false; right = false; up = false; down = false;
            upleft = false; upright = false; downleft = false; downright = false;
        ...
        public static bool connect(int b1, int b2)
        {
            return (b1 != 0 && b1 == b2 ||
                b1 == 1 && b2 == 72 ||
                b2 == 1 && b1 == 72 ||
        ...
        public static bool connect(int x1, int y1, int x2, int y2, int[,] blocks)
        {
            return y1 > 0 && y1 < blocks.Length - 1 && connect(blocks[y1,mod(x1, blocks.GetUpperBound(1))], blocks[y2,mod(x2, blocks.GetUpperBound(1))]);
            /*        int WIDTH = blocks[0].length;
                    int HEIGHT = blocks.length;
                    bool[] blockcds = TerrariaClone.getBLOCKCDS();
        ...
        public static Byte[,] generate3(int[,] blocks, double[] terrain, double[] stonelayer, TerrariaClone inst)
        {
            int x, y;
        ...
        public static bool[,] blob(int cwidth, int cheight, double erosion)
        {
            int x, y;
            coordlist = new bool[cwidth * 2 + 1,cheight * 2 + 1];
                ...
                for (x = -cwidth; x <= cwidth; x++)
                {
                    for (y = -cheight; y <= cheight; y++)
                    {
                        if ((x + cwidth <= 0 || !coordlist[x + cwidth - 1,y + cheight]) &&
                            (x + cwidth + 1 >= coordlist.GetUpperBound(0) || !coordlist[x + cwidth + 1,y + cheight]) &&
                            (y + cheight <= 0 || !coordlist[x + cwidth,y + cheight - 1]) &&
                            (y + cheight + 1 >= coordlist.GetUpperBound(1) || !coordlist[x + cwidth,y + cheight + 1]))
                        {
